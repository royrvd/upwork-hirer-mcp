...
export async function handleAuthTool(name: string, _args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    case "auth_status": {
      const t = loadTokens();
      if (!t.access_token) return { authenticated: false, message: "Call auth_start to log in." };
      return { authenticated: true, expires_at: t.expires_at ? new Date(t.expires_at).toISOString() : "unknown" };
    }

    case "auth_start": {
      if (!CLIENT_ID || !CLIENT_SECRET) throw new Error("UPWORK_CLIENT_ID and UPWORK_CLIENT_SECRET must be set.");
      const url = buildAuthUrl();
      waitForCallback().then(async ({ code, state }) => {
        const t = loadTokens();
        if (state !== t.pending_state) throw new Error("State mismatch");
