...
export async function handleContractTool(name, args) {
    switch (name) {
      ...
      `, { ...(pagination ? { pagination } : {}) });
            const rooms = roomData.roomList?.edges?.map((e) => e.node) || [];
            const contractIds = [...new Set(rooms.map((r) => r.contractId).filter(Boolean))];
            if (!contractIds.length)
                return { message: "No contracts found via rooms.", rooms };
      ...
      `, { id: args.contract_id });
            ...
            const milestones = (terms?.fixedPriceTerms || []).flatMap((t) => t.milestones || []);
      ...
      `, { contractId: args.contract_id, message: args.message || null });
